            ...
            <div className="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              <div className="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-medium text-gray-900 dark:text-white">Connect Wallet</h3>
                  <button
                    onClick={() => setIsModalOpen(false)}
                    className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
                    title="Close wallet selection modal"
                  >
                    <XMarkIcon className="w-6 h-6" />
                  </button>
                </div>

                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Choose a wallet to connect to DecentralAI</p>
                ...
                <div className="space-y-3">
                  {wallets?.map((wallet) => (
                    ...
                    >
                      {!isKmd(wallet.id) && (
                        <img src={wallet.metadata.icon} alt={`${wallet.metadata.name} icon`} className="w-8 h-8 rounded-lg" />
                      )}
                      {isKmd(wallet.id) && (
